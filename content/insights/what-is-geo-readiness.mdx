...
Publish an [`llms.txt`](https://llmstxt.org/) at the domain root. Add [FAQ schema](https://schema.org/FAQPage) to the three pages that answer real customer questions. Rewrite one service page so it leads with a one-sentence answer. Measure citations for six weeks. Small moves, compounded, are what shift the score.
